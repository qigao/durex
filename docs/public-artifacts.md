...
| Artifact | Contract |
| --- | --- |
| `shared-common` | immutable shared response/error model used by Durex runtime adapters |
...
The examples below use the intended `0.1.0` release coordinate. Repository PR CI proves the same graph from an isolated staged Maven repository before public deployment.
...
String value = ok.result();
throw new ApiException("missing", ErrorCode.ENTITY_NOT_FOUND);
```

`RespData<T>` and `ErrorResponse` are immutable Java records. The public HTTP payload does not expose an originating implementation class/method. One `ApiException` owns one stable `ErrorResponse` occurrence, including its error id and timestamp.

The Spring HTTP adapter supplies Durex's `ApiException` mapping when its auto-configuration is active. `ENTITY_NOT_FOUND` maps to HTTP 404, `EMPTY_PARAM` and `VALUE_ERROR` map to HTTP 400, and other/unclassified Durex errors map to HTTP 500.
...
The initial supported API includes the complete HTTP error model (`ApiException`, `ErrorCode`, immutable `ErrorResponse`, immutable `RespData`, `DurexHttpExceptionHandler`), the three messaging annotations, and the Redis codec/failure-policy contracts including `RedisStreamListenerFailure`.
...
The check is directional during development: every baseline line must still exist in the current staged artifact. This catches removed classes/members, incompatible JVM descriptor changes, and visibility/class-declaration changes while allowing additive API growth. Before a stable release, release verification requires the generated current signature set to exactly equal the committed release baseline. Types classified as `runtime` are intentionally excluded from the user API compatibility promise.
...
Durex is distributed under Apache License 2.0. The repository-root `LICENSE` is canonical and `gradle/durex-publication.gradle` fixes the matching Maven POM metadata; legacy license override properties/environment variables are rejected.

PR CI does not require a signing secret. When a signing key is present for a real release, publishing the Maven publication creates and publishes the `.asc` signatures for the publication artifacts/metadata.

Durex does not configure the retired OSSRH service. Release automation targets the Central Publisher Portal bundle/API path after the exact release SHA has passed staging and compatibility checks.
...
`PublicApiBaseline.java` is a compile-time usability fixture for the promised API, including the immutable HTTP record accessors. The signature baseline adds deterministic JVM-level compatibility protection on top of that consumer compile proof.
