...
public class EventAnnotationHandlersManager {
  ...
  /**
   * This method will obtain all handled methods with at least one event handling annotation. In
   * case some method has more than one handling annotation it will be obtained twice, one for each
   * handler.
   *
  ...
  public List<HandledMethod> getHandledMethodsFrom(Class<?> classListener) {
    return handledMethodsFrom(classListener).stream()
        .flatMap(
            method ->
                getHandlers(method).stream().map(handler -> new HandledMethod(method, handler)))
        .collect(Collectors.toList());
  }
