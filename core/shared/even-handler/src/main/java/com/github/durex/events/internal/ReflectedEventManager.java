...
import com.github.durex.events.internal.annotations.EventAnnotationHandlersManager;
import com.github.durex.events.internal.annotations.EventHandlerAnnotationEventHandler;
...
public class ReflectedEventManager implements EventManager {
  ...
  /**
   * Internal management of event handling annotations. To register annotations it requires an event
   * annotation handler.
   */
  private final EventAnnotationHandlersManager handlersManager =
      new EventAnnotationHandlersManager() {
        {
  ...
  /**
   * Register event listener at the EventManager to fire its event when some handled event went
   * called.
   *
  ...
  public <L extends EventListener> void registerEvents(L listener) {
    ...
    List<HandledMethod> eventHandlerMethods =
        handlersManager.getHandledMethodsFrom(listener.getClass());

    // Map event handler methods to a RegisterEventHandler instance.
    List<RegisteredEventHandler> eventHandlers =
  ...
  public <L extends EventListener> void unregisterEvents(L listener) {
    ...
    List<RegisteredEventHandler> eventHandlers =
        eventManagement.getRegisteredEvents().stream()
  ...
  public <E extends Event> void callEvent(E event) {
    ...
    for (RegisteredEventHandler eventHandler :
        eventManagement.getEventHandlerFor(EventDispatchEvent.class)) {
    ...
    for (RegisteredEventHandler eventHandler : eventManagement.getEventHandlerFor(event)) {
      // Skip event handling execution for these events that were canceled and are not flagged to
      // 'ignore event cancellation'.
