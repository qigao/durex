...
public class EventAnnotation extends AbstractProcessor {
    ...
    roundEnv.getElementsAnnotatedWith(InComing.class).stream()
        ...
        .forEach(
            element -> {
              ...
              var codeNameInfo = Helper.buildCodeNameInfo(classInfo, methodInfo);
              ...
              if ("NoGroup".equals(topicInfo.getGroup())) {
                ...
                var subEventRunnerClassName = targetClassName + "Config";
                subEventRunnerCodeGenerator(subEventRunnerClassName, javaFile, topicInfo);
              } else {
  ...
  /**
   * Generate a java file with the given package name and class name. {@link
   * RedisCodeGenerator#subEventRunnerCodeGenerator}
   *
   * @param lifecycleClassName class name of the generated subscriber
   * @param filer filer to write the generated java file
   */
  @SneakyThrows
  private void subEventRunnerCodeGenerator(
      String lifecycleClassName, Filer filer, TopicInfo topicInfo) {
    final Writer writer = filer.createSourceFile(lifecycleClassName).openWriter();
    redisCodeGenerator.subEventRunnerCodeGenerator(topicInfo, writer);
    writer.close();
  }
